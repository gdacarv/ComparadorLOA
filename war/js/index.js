...
function loadFuncoes(id1, id2) {
	window.loadingFuncoes = true;
	var xmlhttp;
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			...
			window.loadingFuncoes = false;
			if(!window.loadingAnos){
				makeClean("dark");
				makeClean("carregando");
			}
		}
	}
	xmlhttp.open("GET", "http://comparadorloa.appspot.com/funcao", true);
	xmlhttp.send();
...
function loadSubfuncoes(id, combobox) {
	...
	xmlhttp.open("GET", "http://comparadorloa.appspot.com/subfuncao?Funcao=" + combobox.value, true);
	xmlhttp.send();
	makeVisibleWithoutScroll("dark");
	makeVisibleWithoutScroll("carregando");
}

function loadAnos(id1, id2) {
	window.loadingAnos = true;
	var xmlhttp;
	if (window.XMLHttpRequest) {// code for IE7+, Firefox, Chrome, Opera, Safari
		xmlhttp = new XMLHttpRequest();
	} else {// code for IE6, IE5
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			document.getElementById(id1).innerHTML = xmlhttp.responseText;
			document.getElementById(id2).innerHTML = xmlhttp.responseText;
			window.loadingAnos = false;
			if(!window.loadingFuncoes){
				makeClean("dark");
				makeClean("carregando");
			}
		}
	}
	xmlhttp.open("GET", "http://comparadorloa.appspot.com/anos", true);
	xmlhttp.send();
	makeVisibleWithoutScroll("dark");
	makeVisibleWithoutScroll("carregando");
}
...
function comparar() {
	...
	xmlhttp.open("GET", "http://comparadorloa.appspot.com/comparar?Funcao1=" + domNode1.value + "&Subfuncao1="
			+ document.getElementById("first-category-second").value
			+ "&Funcao2=" + domNode2.value + "&Subfuncao2="
			+ document.getElementById("second-category-second").value
			+ "&inicio="
			+ document.getElementById("first-category-third").value + "&fim="
			+ document.getElementById("second-category-third").value
			+ "&Funcao1Nome=" + selected_text1 + "&Funcao2Nome="
			+ selected_text2, true);
	xmlhttp.send();
	makeVisibleWithoutScroll("dark");
	makeVisibleWithoutScroll("carregando");
...
function verificarCampos() {
	...
	if (document.getElementById("first-category-primary").value == 0
			|| document.getElementById("second-category-primary").value == 0) {
		alert("Selecione funções");
		return false;
	}
	if (document.getElementById("first-category-primary").value == document
			.getElementById("second-category-primary").value
			&& document.getElementById("first-category-second").value == document
					.getElementById("second-category-second").value) {
